  ...
  public void startElement(String namespaceUri, String localName, String qName,
    ...
    if (namespaceUri.equals(EdiXmlAdapter.NAMESPACE_URI)) {
      if (localName.equals(EdiXmlAdapter.SEGMENT_ELEMENT)) {
        final String segmentIdentifier = attributes.getValue(EdiXmlAdapter.TYPE_ATTRIBUTE);
        try {
          if (startedLoopSegment) {
            // Loops are serialized in a special way. The starting segment has
// all the
            // following segments as children. So, the first loop segment needs
// special
            // handling.
            if (hasSegmentContents) {
              writer.endSegment();
