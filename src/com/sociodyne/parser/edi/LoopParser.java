...
public class LoopParser extends SegmentParser {
	...
	LoopParser(@Assisted Tokenizer tokenizer, @Assisted EdiLocation location,
			...
			Set<String> nonTerminalSegments) {
	...
	private Token parseChildSegments(String loopIdentifier, Token token)
		...
		while (token != null) {
			...
			if (token.getType() == Token.Type.WORD &&
				(token.getValue().equals(loopIdentifier)
						|| ! nonTerminalSegments.contains(token.getValue()))) {
			...
			// TODO(jkinner): This needs to produce a segment parser for the identified
			// segment
			SegmentParser segmentParser = segmentParserFactory.create(tokenizer, location,
					handler, token.getValue());
