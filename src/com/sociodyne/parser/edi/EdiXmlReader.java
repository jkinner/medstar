...
import com.sociodyne.debug.LoggingInvocationHandler;
...
public class EdiXmlReader implements XMLReader {
	...
	public static final String SEGMENT_TERMINATOR_PROPERTY =
		"http://www.sociodyne.com/xmlns/edi/segmentTerminator";

	/** Property URI for setting the default element separator. */
	public static final String ELEMENT_SEPARATOR_PROPERTY =
		"http://www.sociodyne.com/xmlns/edi/elementSeparator";

	/** Property URI for setting the default subelement separator. */
	public static final String SUBELEMENT_SEPARATOR_PROPERTY =
		"http://www.sociodyne.com/xmlns/edi/subelementSeparator";
	...
	private boolean defaultUseNamespacePrefixes = false;
	...
	protected void parseReader(Reader reader) throws IOException, SAXException {
		...
		EdiLocation location = new EdiLocation("ISA", 0);
		Tokenizer tokenizer = new Tokenizer(reader, configuration, location);
		...
		EdiHandler handler = new EdiXmlAdapter(contentHandler);
...
//				new LoggingInvocationHandler(handler, System.err));
		EdiHandler isaFilterHandler = new IsaEdiHandler(configuration, handler);
		SegmentParser isaSegmentParser = segmentParserFactory.create(location, tokenizer,
				isaFilterHandler);
		try {
		...
		SegmentListParser segmentListParser = segmentListParserFactory.create(location, tokenizer,
				handler);
