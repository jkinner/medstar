...
/**
 * Servlet that enables customers to upload files of various types and have them processed.
...
public class UploadServlet extends HttpServlet {
	...
	public UploadServlet(MultipartChecker checker, Provider<ServletFileUpload> uploadProvider,
		...
		if (log.isLoggable(Level.FINE)) {
			for (Map.Entry<String,UploadHandler> handlersEntry : handlers.entrySet()) {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (checker.isMultipartContent(req)) {
			ServletFileUpload fileUpload = uploadProvider.get();
			...
			try {
				while (files.hasNext()) {
					try {
						FileItemStream fileStream = files.next();
						String contentType = fileStream.getContentType();
						UploadHandler handler = handlers.get(contentType);
						if (handler != null) {
							try {
