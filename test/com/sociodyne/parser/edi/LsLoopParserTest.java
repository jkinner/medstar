...
public class LsLoopParserTest extends MockEdiParserTest {
	...
	public void testSingleElementLoop_succeeds() throws Exception {
		...
		expect(segmentParser.parse(eq(new Token(Token.Type.WORD, "DTP"))))
			.andReturn(Token.SEGMENT_TERMINATOR);
		expect(segmentParserFactory.create(anyObject(Tokenizer.class),
			eq(location), eq(handler), eq("LE")))
			.andReturn(segmentParser);
		expect(segmentParser.parse(eq(new Token(Token.Type.WORD, "LE"))))
			.andReturn(Token.SEGMENT_TERMINATOR);
		...
		LsLoopParser loopParser = new LsLoopParser(tokenizer, location, handler,
	...
	public void testMultipleElementLoop_succeeds() throws Exception {
		...
		expect(segmentParser.parse(eq(new Token(Token.Type.WORD, "DTP"))))
			.andReturn(Token.SEGMENT_TERMINATOR);
		expect(segmentParserFactory.create(anyObject(Tokenizer.class),
			eq(location), eq(handler), eq("EQ")))
			.andReturn(segmentParser);
		expect(segmentParser.parse(eq(new Token(Token.Type.WORD, "EQ"))))
			.andReturn(Token.SEGMENT_TERMINATOR);
		expect(segmentParserFactory.create(anyObject(Tokenizer.class),
			eq(location), eq(handler), eq("LE")))
			.andReturn(segmentParser);
	...
	public void testSingleElementLoop_withInnerLoop_succeeds() throws Exception {
		...
		expect(elementListParser.parse(eq(Token.ELEMENT_SEPARATOR)))
			.andReturn(Token.SEGMENT_TERMINATOR);
		expect(segmentParserFactory.create(anyObject(Tokenizer.class),
			eq(location), eq(handler), eq("EB")))
			.andReturn(segmentParser);
		expect(segmentParser.parse(eq(new Token(Token.Type.WORD, "EB"))))
			.andReturn(Token.word("LE"));
		expect(segmentParserFactory.create(anyObject(Tokenizer.class),
			eq(location), eq(handler), eq("LE")))
			.andReturn(segmentParser);
		expect(segmentParser.parse(eq(new Token(Token.Type.WORD, "LE"))))
